...
const PREVIEW_PROMPT = (race) => `You are an F1 quiz master creating a Thursday preview quiz about the history of the ${race} Grand Prix circuit for fans ahead of this weekend's race.
...
export default async function handler(req, res) {
  ...
  try {
    const prompt = mode === "preview" ? PREVIEW_PROMPT(race) : REVIEW_PROMPT();
    const fullText = await callAnthropic({
      model: "claude-sonnet-4-6",
      maxTokens: 8192,
      prompt,
      // Review (post-race) needs live results, so it searches the web.
      ...(mode === "review"
    ...
    res.status(200).json({ race: resolvedRace, mode, questions });
  } catch (err) {
