  ...
  try {
    ...
    const message = await anthropic.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `You are an expert Formula 1 analyst. Predict the race result for the upcoming 2026 ${race} (Round ${round}).
...
The 2026 grid: McLaren (Norris, Piastri), Ferrari (Leclerc, Hamilton), Red Bull (Verstappen, Lawson), Mercedes (Russell, Antonelli), Williams (Sainz, Albon), Aston Martin (Alonso, Stroll), Alpine (Gasly, Doohan), Haas (Ocon, Bearman), RB (Tsunoda, Hadjar), Audi (Hülkenberg, Bottas), Cadillac (Herta, Armstrong).
...
Return ONLY valid JSON with no markdown or code fences:
{
  ...
  "championshipImpact": "What this race could mean for the title fight (max 20 words)"
    ...
    let jsonText = textBlock.text.trim().replace(/^```(?:json)?\s*/i, "").replace(/\s*```$/, "");
